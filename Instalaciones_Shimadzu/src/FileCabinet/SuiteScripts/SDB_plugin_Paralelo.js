 ...
 function customizeGlImpact(transactionRecord, standardLines, customLines, book) {
            ...
            if (installed || installed == "T") {//Si el inventoryadjustment (-) se crea al instalar 
                ...
                for (var i = 0; i < standardLines.getCount(); i++) {
                    ...
                    if (parseFloat(standardLines.getLine(i).getDebitAmount()) > 0) {
                        ...
                        var location = returnAccounForLocation(currentLocation)
    ...
    if (recordType == 'invoice') {
        ...
        try {
            ...
            var fechaInst = transactionRecord.getFieldValue('custbody_sdb_fecha_instalada');
            var fechaNoinst = transactionRecord.getFieldValue('custbody_sdb_fecha_no_instalada');
            nlapiLogExecution("AUDIT", "getGlImpact -INV", JSON.stringify(getGlImpact(recordType, rcdId)));

            var custpostGroup;

            if (customer) customerData = nlapiLookupField('customer', customer, ['custentity_sdb_posting_group']); custpostGroup = customerData.custentity_sdb_posting_group;
            ...
            var invoiceLinesGL = getGlImpact(recordType, rcdId)
            nlapiLogExecution("DEBUG", "invoiceLinesGL---", JSON.stringify(invoiceLinesGL));
            ...
            nlapiLogExecution("AUDIT", " standardLines.getCount() ", standardLines.getCount());
            nlapiLogExecution("AUDIT", "invoiceLinesGL.length", invoiceLinesGL ? invoiceLinesGL.length : 'No length');
            ...
            //         var currentLocation = standardLines.getLine(i).getLocationId();
            //         var entity = standardLines.getLine(i).getEntityId();
            //         var validateACC = iter_3ro_acc.indexOf(currentAcc);// Si pertenece a cuenta de tercero || intercompany
            //         var item;
            //         if (invoiceLinesGL && invoiceLinesGL.length && i > 0 && i < (invoiceLinesGL.length - 1)) {
            //             nlapiLogExecution("DEBUG", "I-0", i);
            //             item = invoiceLinesGL.filter(function (pos) {
            //                 return (Number(pos.line) == i);
            //             });
            //         }
            //         var itemGroup;

            //         nlapiLogExecution("DEBUG", "item--", JSON.stringify(item));
            //         if (item && item.length) itemGroup = getItemPostGroup(item[0].item)
            //         nlapiLogExecution("DEBUG", "itemGroup", JSON.stringify(itemGroup));
            //         var prod_post_group = itemGroup ? itemGroup.custitem_sdb_gen_prod_post_group : '';
            //         nlapiLogExecution("DEBUG", "prod_post_group---", prod_post_group);
            //         var entity = standardLines.getLine(i).getEntityId();
            //         var accCostToSet = getAccCost(custpostGroup, currentAcc)//'TRAE LA CUENTA DE COST SEGUN POSTING GROUP Y ACCOUNT'
            //         var groupAccounts = setGeneralGroupAccount(custpostGroup, currentAcc, prod_post_group)//'TRAE LA CUENTA DE INVENTARIO SEGUN POSTING GROUP Y ACCOUNT'
            //         nlapiLogExecution("DEBUG", "accCostToSet---", accCostToSet);
            //         nlapiLogExecution("DEBUG", "groupAccounts---", JSON.stringify(groupAccounts));
            //         // nlapiLogExecution("AUDIT", " currentAcc <<>>", currentAcc);
            //         // if (i > 0) {
            //         if (Number(standardLines.getLine(i).getCreditAmount()) > 0 && groupAccounts/*currentAcc == configData.custrecord_sdb_inv_puente_acc*/) {
            ...
            //             if (entity && accType != 'AcctRec') newLine.setEntityId(parseInt(entity))
            //             if (standardLines.getLine(i).getSegmentValueId('cseg1')) newLine.setSegmentValueId('cseg1', standardLines.getLine(i).getSegmentValueId('cseg1'));
            //             if (groupAccounts.salesAcc) newLine.setAccountId(parseInt(groupAccounts.salesAcc));
            //             // if (isInterCompany != -1) newLine.setAccountId(parseInt(configData.custrecord_sdb_inv_art_intercom));//41200 Sales : Sales, Intercompany
            //             // if (terceros != -1) newLine.setAccountId(parseInt(configData.custrecord_sdb_inv_art_3ro));//41100 Sales : Sales, 3rd Company
            ...
            if (instalacion == 'F' && installed == 'F') {
                ...
                for (var i = 0; i < standardLines.getCount(); i++) {
                    ...
                    nlapiLogExecution("DEBUG", "item-Data", JSON.stringify(item));
                    var itemGroup;
                    if (item && item.length) itemGroup = getItemPostGroup(item[0].item);
                    var itemtype;
                    if (itemGroup) itemtype = itemGroup.recordType;
                    var custitem_sdb_gen_prod_post_group = '';
                    if (itemGroup) custitem_sdb_gen_prod_post_group = itemGroup.custitem_sdb_gen_prod_post_group;
                    var amountValue = 1;
                    //   var accSet = setCustGroupAccount(custpostGroup, currentAcc);
                    var accCostToSet = getAccCost(custpostGroup, currentAcc)//'TRAE LA CUENTA DE COST SEGUN POSTING GROUP Y ACCOUNT'  
                    var groupAccounts = setGeneralGroupAccount(custpostGroup, currentAcc, custitem_sdb_gen_prod_post_group)//'TRAE LA CUENTA DE INVENTARIO SEGUN POSTING GROUP Y ACCOUNT'    
                    nlapiLogExecution("DEBUG", "groupAccounts-", groupAccounts);
                    nlapiLogExecution("DEBUG", "itemtype >>", itemtype);


                    if (/*currentAcc == configData.custrecord_sdb_cuenta_costo*/ accCostToSet) { //51999 Cost of Sales : Cost of Sales, (Interim)
                        ...
                        if (Number(standardLines.getLine(i).getCreditAmount()) > 0) {
                            ...
                            if (entity && accType != 'AcctRec') newLine.setEntityId(parseInt(entity))
                            if (standardLines.getLine(i).getSegmentValueId('cseg1')) newLine.setSegmentValueId('cseg1', standardLines.getLine(i).getSegmentValueId('cseg1'));
                            if (accCostToSet) newLine.setAccountId(parseInt(accCostToSet)); //SET Cost Account
                            //  if (isInterCompany != -1) newLine.setAccountId(parseInt(configData.custrecord_sdb_inv_cost_ic));"51450 Cost of Sales : Cost of Sales, Intercompany"
                            //  if (terceros != -1) newLine.setAccountId(parseInt(configData.custrecord_sdb_inv_cost_3ro)); "51400 Cost of Sales : Cost of Sales, 3rd Company"
                    ...
                    } else if (/*currentAcc == configData.custrecord_sdb_inv_item_acc*/groupAccounts || itemtype == "discountitem") {// Si la cuenta es puente ("41799 Sales : Sales, (Interim)) se balancea y se setea nueva linea con cuenta descuento (intercompany o terceros)
                        ...
                        if (Number(standardLines.getLine(i).getCreditAmount()) > 0) {
                            ...
                            if (itemtype == "discountitem") {//SI ES UNA LINEA DE DESCUENTO 
                                var newLine = customLines.addNewLine();
                                ...
                                if (entity && accType != 'AcctRec') newLine.setEntityId(parseInt(entity))
                                if (standardLines.getLine(i).getSegmentValueId('cseg1')) newLine.setSegmentValueId('cseg1', standardLines.getLine(i).getSegmentValueId('cseg1'));
                                // if (groupAccounts.discountAcc) newLine.setAccountId(parseInt(groupAccounts.discountAcc));
                                // if (isInterCompany != -1) newLine.setAccountId(parseInt(configData.custrecord_sdb_inv_art_intercom));
                                newLine.setAccountId(parseInt(configData.custrecord_sdb_inv_item_acc));
                                // if (terceros != -1) newLine.setAccountId(parseInt(configData.custrecord_sdb_discount_3ro_acc));
                                continue;
                            }
                            ...
                            nlapiLogExecution("DEBUG", "custrecord_sdb_inv_item_acc  else if", standardLines.getLine(i).getDebitAmount());
                            ...
                            if (itemtype == "discountitem") {//SI ES UNA LINEA DE DESCUENTO 
                                ...
                                if (entity && accType != 'AcctRec') newLine.setEntityId(parseInt(entity))
                                if (standardLines.getLine(i).getSegmentValueId('cseg1')) newLine.setSegmentValueId('cseg1', standardLines.getLine(i).getSegmentValueId('cseg1'));
                                if (groupAccounts.salesAcc) newLine.setAccountId(parseInt(groupAccounts.discountAcc));
                                // if (isInterCompany != -1) newLine.setAccountId(parseInt(configData.custrecord_sdb_inv_art_intercom));
                                // if (isInterCompany != -1) newLine.setAccountId(parseInt(configData.custrecord_sdb_discount_inter_acc));
                                // if (terceros != -1) newLine.setAccountId(parseInt(configData.custrecord_sdb_discount_3ro_acc));
                        ...
                        var chargeAcc = getChargeAcc(custpostGroup, currentAcc); //TRAE LA CUENTA DE CHARGE SEGUN POSTING GROUP
                        if (!chargeAcc) continue;
                        if (Number(standardLines.getLine(i).getCreditAmount()) > 0) {
                            ...
                            if (chargeAcc) newLine.setAccountId(parseInt(chargeAcc));
                        } else if (Number(standardLines.getLine(i).getDebitAmount()) > 0) {
            ...
            } else if (instalacion == 'T' /*&& installed == 'F'*/) {// Verificar si esto de que necesite instalacion y no este instalada || !diff falsa
                ...
                for (var i = 0; i < standardLines.getCount(); i++) {
                    ...
                    var itemGroup;
                    nlapiLogExecution("DEBUG", "item--", JSON.stringify(item));
                    if (item.length) itemGroup = getItemPostGroup(item[0].item);
                    nlapiLogExecution("AUDIT", "item[0].item", item[0].item);
                    if (item[0].item) var reportNumber = getReport(createFrom, item[0].item) || '';
                    nlapiLogExecution("AUDIT", "reportNumber", JSON.stringify(reportNumber));
                    nlapiLogExecution("DEBUG", "itemGroup", JSON.stringify(itemGroup));
                    var itemtype;
                    if (itemGroup) itemtype = itemGroup.recordType;
                    var entity = standardLines.getLine(i).getEntityId();
                    var accCostToSet = getAccCost(custpostGroup, currentAcc)//'TRAE LA CUENTA DE COST SEGUN POSTING GROUP Y ACCOUNT'
                    var groupAccounts = setGeneralGroupAccount(custpostGroup, currentAcc, itemGroup.custitem_sdb_gen_prod_post_group)//'TRAE LA CUENTA DE INVENTARIO SEGUN POSTING GROUP Y ACCOUNT'
                    nlapiLogExecution("DEBUG", "accCostToSet---", accCostToSet);
                    nlapiLogExecution("DEBUG", "groupAccounts---", JSON.stringify(groupAccounts));
                    nlapiLogExecution("DEBUG", "Number(standardLines.getLine(i).getDebitAmount())", Number(standardLines.getLine(i).getDebitAmount()));

                    var date = new Date(); // Get the current date
                    var formattedDate = nlapiDateToString(date);
                    var fechaInst = transactionRecord.getFieldValue('custbody_sdb_fecha_instalada') ? transactionRecord.getFieldValue('custbody_sdb_fecha_instalada') : formattedDate;
                    var fechaNoinst = transactionRecord.getFieldValue('custbody_sdb_fecha_no_instalada') ? transactionRecord.getFieldValue('custbody_sdb_fecha_no_instalada') : formattedDate;
                    nlapiLogExecution("DEBUG", "fechaInst---", fechaInst);
                    // if (i > 0) {
                    //  var accountSet = setGeneralGroupAccount(custpostGroup, currentAcc)
                    if (Number(standardLines.getLine(i).getCreditAmount()) > 0 && groupAccounts/*currentAcc == ctaPuenteInv*/) {// Si la cuenta es puente ("41799 Sales : Sales, (Interim)) se balancea y se setea nueva linea con cuenta descuento (intercompany o terceros)
                        nlapiLogExecution("DEBUG", "instalacion not installed1", instalacion);
                        nlapiLogExecution("DEBUG", "I-1", i);
                        ...
                        newLine.setAccountId(parseInt(currentAcc));
                        newLine.setLocationId(parseInt(currentLocation));
                        if (standardLines.getLine(i).getMemo()) { newLine.setMemo(standardLines.getLine(i).getMemo() + '  (' + fechaNoinst + ')') } else { newLine.setMemo('(' + fechaNoinst + ')') }
                        if (standardLines.getLine(i).getClassId()) newLine.setClassId(standardLines.getLine(i).getClassId());
                        if (standardLines.getLine(i).getDepartmentId()) newLine.setDepartmentId(standardLines.getLine(i).getDepartmentId());
                        if (entity && accType != 'AcctRec') newLine.setEntityId(parseInt(entity))
                        if (standardLines.getLine(i).getSegmentValueId('cseg1')) newLine.setSegmentValueId('cseg1', standardLines.getLine(i).getSegmentValueId('cseg1'));
                        ...
                        //    if (isInterCompany != -1) newLine.setAccountId(parseInt(configData.custrecord_sdb_inv_art_intercom));
                        if (groupAccounts.salesAcc) newLine.setAccountId(parseInt(groupAccounts.salesAcc));//TO DO                                                                       <--'SE COMENTO PARA NO REPETIR'
                        ...
                        //if (isInterCompany != -1) newLine.setAccountId(parseInt(configData.custrecord_sdb_inv_art_intercom));
                        if (groupAccounts.salesAcc) newLine.setAccountId(parseInt(groupAccounts.salesAcc));
                        nlapiLogExecution("DEBUG", "groupAccounts.isWarranty", groupAccounts.isWarranty);
                        //    if (groupAccounts.isWarranty == 'T') continue;
                        var newLine = customLines.addNewLine();
                        newLine.setCreditAmount(Number(standardLines.getLine(i).getCreditAmount()));
                        newLine.setLocationId(parseInt(currentLocation));
                        if (standardLines.getLine(i).getMemo()) { newLine.setMemo(standardLines.getLine(i).getMemo() + '  (' + fechaNoinst + ')') } else { newLine.setMemo('(' + fechaNoinst + ')') }
                        ...
                        if (entity && accType != 'AcctRec') newLine.setEntityId(parseInt(entity))
                        if (standardLines.getLine(i).getSegmentValueId('cseg1')) newLine.setSegmentValueId('cseg1', standardLines.getLine(i).getSegmentValueId('cseg1'));
                        if (groupAccounts.accWithInstall && groupAccounts.isWarranty == 'F') newLine.setAccountId(parseInt(groupAccounts.accWithInstall));
                        if (configData.custrecord_sdb_momento_vta_ci && groupAccounts.isWarranty == 'T') newLine.setAccountId(parseInt(configData.custrecord_sdb_momento_vta_ci));
                        nlapiSubmitField(recordType, rcdId, 'custbody_sdb_fecha_no_instalada', fechaNoinst)
                        //  transactionRecord.setFieldValue('custbody_sdb_fecha_no_instalada', fechaNoinst)

                        if (!installed || installed == 'F') continue;

                        var newLine = customLines.addNewLine();
                        newLine.setCreditAmount(Number(standardLines.getLine(i).getCreditAmount()));
                        newLine.setLocationId(parseInt(currentLocation));
                        if (standardLines.getLine(i).getMemo()) { newLine.setMemo(standardLines.getLine(i).getMemo() + '   (' + (reportNumber.length ? reportNumber[0].dateInstalled : '') + ') Report : ' + (reportNumber.length ? reportNumber[0].report : '')) } else { newLine.setMemo('(' + (reportNumber.length ? reportNumber[0].dateInstalled : '') + ') Report : ' + (reportNumber.length ? reportNumber[0].report : '')) }
                        if (standardLines.getLine(i).getClassId()) newLine.setClassId(standardLines.getLine(i).getClassId());
                        if (standardLines.getLine(i).getDepartmentId()) newLine.setDepartmentId(standardLines.getLine(i).getDepartmentId());
                        if (entity && accType != 'AcctRec') newLine.setEntityId(parseInt(entity))
                        if (standardLines.getLine(i).getSegmentValueId('cseg1')) newLine.setSegmentValueId('cseg1', standardLines.getLine(i).getSegmentValueId('cseg1'));
                        ...
                        nlapiSubmitField(recordType, rcdId, 'custbody_sdb_fecha_instalada', fechaInst)
                        //transactionRecord.setFieldValue('custbody_sdb_fecha_instalada', fechaInst);

                    } else if (Number(standardLines.getLine(i).getDebitAmount()) > 0) {
                        nlapiLogExecution("DEBUG", "I-2", i);
                        //  accountSet = setGeneralGroupAccount(custpostGroup, currentAcc)
                        ...
                        if (entity && accType != 'AcctRec') newLine.setEntityId(parseInt(entity))
                        if (standardLines.getLine(i).getSegmentValueId('cseg1')) newLine.setSegmentValueId('cseg1', standardLines.getLine(i).getSegmentValueId('cseg1'));
                        if (groupAccounts.accWithInstall) newLine.setAccountId(parseInt(groupAccounts.accWithInstall));
    ...
    if (recordType == "creditmemo") {
        ...
        try {
            ...
            for (var i = 0; i < standardLines.getCount(); i++) {
                ...
                var acclocation = returnAccounForLocation(locationline)
    ...
    if (recordType == 'itemfulfillment') {
        ...
        try {
            ...
            var itemFLinesGL = getGlImpact(recordType, rcdId)
            nlapiLogExecution("AUDIT", "itemFLinesGL", JSON.stringify(itemFLinesGL));
            ...
            var date = new Date(); // Get the current date
            var formattedDate = nlapiDateToString(date);
            var fechaInst = transactionRecord.getFieldValue('custbody_sdb_fecha_instalada') ? transactionRecord.getFieldValue('custbody_sdb_fecha_instalada') : '';
            var fechaNoinst = transactionRecord.getFieldValue('custbody_sdb_fecha_no_instalada') ? transactionRecord.getFieldValue('custbody_sdb_fecha_no_instalada') : formattedDate;
            nlapiLogExecution("DEBUG", "fechaInst---", fechaInst);
            ...
            if (installation.custbody_sdb_requires_installation == 'T') {/*IF  INSTALLED OR REQUIERE*/
                    ...
                    for (var i = 0; i < standardLines.getCount(); i++) {
                        ...
                        var itemFGL = itemFLinesGL.filter(function (pos) {
                            return (pos.line == i);
                        });
                        nlapiLogExecution("AUDIT", "itemFGL", JSON.stringify(itemFGL));
                        var reportNumber = '';
                        if (itemFGL[0].item) reportNumber = getReport(createdfrom, itemFGL[0].item) || '';
                        var accType = nlapiLookupField('account', currentAcc, 'type');
                        var currentLocation = standardLines.getLine(i).getLocationId();
                        var entity = standardLines.getLine(i).getEntityId();
                        var locationline = standardLines.getLine(i).getLocationId();
                        //   if (locationline) locationline = nlapiLookupField('location', locationline, 'name');
                        var fechaInst = reportNumber ? reportNumber.dateInstalled : '';
                        var acclocation = returnAccounForLocation(locationline)
                        if (Number(standardLines.getLine(i).getCreditAmount()) > 0 && configData.custrecord_sdb_cuenta_costo == currentAcc) {
                            ...
                            newLine.setAccountId(parseInt(currentAcc));
                            newLine.setLocationId(parseInt(currentLocation));
                            if (standardLines.getLine(i).getMemo()) { newLine.setMemo(standardLines.getLine(i).getMemo() + '(' + reportNumber.length ? reportNumber[0].dateInstalled : '' + ' Report:') } else
                                if (standardLines.getLine(i).getClassId()) newLine.setClassId(standardLines.getLine(i).getClassId());
                            if (standardLines.getLine(i).getDepartmentId()) newLine.setDepartmentId(standardLines.getLine(i).getDepartmentId());
                            if (entity && accType != 'AcctRec') newLine.setEntityId(parseInt(entity))
                            if (standardLines.getLine(i).getSegmentValueId('cseg1')) newLine.setSegmentValueId('cseg1', standardLines.getLine(i).getSegmentValueId('cseg1'));
                            ...
                            newLine.setLocationId(parseInt(currentLocation));
                            if (standardLines.getLine(i).getMemo()) { newLine.setMemo(standardLines.getLine(i).getMemo() + '   (' + (reportNumber ? reportNumber[0].dateInstalled : '') + ')Report : ' + (reportNumber ? reportNumber[0].report : '')) } else { newLine.setMemo('(' + (reportNumber ? reportNumber[0].dateInstalled : '') + ') Report : ' + (reportNumber ? reportNumber[0].report : '')) }
                            if (standardLines.getLine(i).getClassId()) newLine.setClassId(standardLines.getLine(i).getClassId());
                            if (standardLines.getLine(i).getDepartmentId()) newLine.setDepartmentId(standardLines.getLine(i).getDepartmentId());
                            if (entity && accType != 'AcctRec') newLine.setEntityId(parseInt(entity))
                            if (standardLines.getLine(i).getSegmentValueId('cseg1')) newLine.setSegmentValueId('cseg1', standardLines.getLine(i).getSegmentValueId('cseg1'));
                            ...
                            nlapiLogExecution("DEBUG", "acclocation <><>1 no installed", acclocation);
                            nlapiLogExecution("DEBUG", "getDebitAmount custrecord_sdb_inv_item_acc no installed", Number(standardLines.getLine(i).getDebitAmount()));
                            var newLine = customLines.addNewLine();
                            newLine.setCreditAmount(Number(standardLines.getLine(i).getDebitAmount()));
                            newLine.setAccountId(parseInt(currentAcc));
                            newLine.setLocationId(parseInt(currentLocation));
                            ...
                            newLine.setLocationId(parseInt(configData.custrecord_sdb_iac_location));
                            newLine.setAccountId(parseInt(configData.custrecord_sdb_adj_iac));
                            //Si no esta intalada continua iteracion  ======= 12/8
                            if (installed == 'F' || reportNumber.length == 0) continue;
                            ...
                            if (Number(standardLines.getLine(i).getCreditAmount()) > 0) {
                                ...
                                nlapiLogExecution("DEBUG", "getCreditAmount custrecord_sdb_inv_item_acc no installed", Number(standardLines.getLine(i).getCreditAmount()));
                                nlapiLogExecution("DEBUG", "acclocation no installed", acclocation);
                                var newLine = customLines.addNewLine();
                                newLine.setDebitAmount(Number(standardLines.getLine(i).getCreditAmount()));
                                newLine.setAccountId(parseInt(currentAcc));
                                newLine.setLocationId(parseInt(currentLocation));
                                ...
                                if (standardLines.getLine(i).getSegmentValueId('cseg1')) newLine.setSegmentValueId('cseg1', standardLines.getLine(i).getSegmentValueId('cseg1'));
                                // newLine.setLocationId(parseInt(configData.custrecord_sdb_iac_location));
                                // newLine.setAccountId(parseInt(configData.custrecord_sdb_adj_iac));configData.custrecord_sdb_cuenta_costo Cambio 22/6/23
                                newLine.setAccountId(parseInt(configData.custrecord_sdb_cuenta_costo));
                                // Si no esta instalada continua la iteracion 12/8
                                if (installed == 'F' || reportNumber.length == 0) continue;
                                // //prueba --- No iria 
                                var newLine = customLines.addNewLine();
                                newLine.setDebitAmount(Number(standardLines.getLine(i).getCreditAmount()));
                                ...
                                if (entity && accType != 'AcctRec') newLine.setEntityId(parseInt(entity))
                                if (standardLines.getLine(i).getSegmentValueId('cseg1')) newLine.setSegmentValueId('cseg1', standardLines.getLine(i).getSegmentValueId('cseg1'));
                                if (configData.custrecord_sdb_adj_iac) newLine.setAccountId(parseInt(configData.custrecord_sdb_adj_iac));
                                // if (acclocation) newLine.setAccountId(parseInt(acclocation));configData.custrecord_sdb_adj_iac
                            } else if (Number(standardLines.getLine(i).getDebitAmount()) > 0) {
                ...
                for (var i = 0; i < standardLines.getCount(); i++) {
                    nlapiLogExecution("ERROR", "installation.custbody_sdb_requires_installation---", installation.custbody_sdb_requires_installation);
                    var currentAcc = standardLines.getLine(i).getAccountId();
                    ...
                    var amountValue = 1;
                    var item;
                    if (itemFLinesGL && itemFLinesGL.length && i > 0 && i < (itemFLinesGL.length - 1)) {
                        nlapiLogExecution("DEBUG", "I-0", i);
                        item = itemFLinesGL.filter(function (pos) {
                            return (Number(pos.line) == i && pos.cusScript == '' && pos.item);
                        });
                    }
                    ...
                    var accCostToSet = getAccCost(custpostGroup, currentAcc)//'TRAE LA CUENTA DE COST SEGUN POSTING GROUP Y ACCOUNT'
                    var groupAccounts = setGeneralGroupAccount(custpostGroup, currentAcc, prod_post_group)//'TRAE LA CUENTA DE INVENTARIO SEGUN POSTING GROUP Y ACCOUNT'
                    // nlapiLogExecution("DEBUG", "accCostToSet-IF", accCostToSet);
                    // nlapiLogExecution("DEBUG", "groupAccounts-IF", JSON.stringify(groupAccounts));
                    var locationline = standardLines.getLine(i).getLocationId();
                    // nlapiLogExecution("ERROR", "locationline", locationline);
                    nlapiLogExecution("ERROR", "itemGroup", itemGroup);
                    var acclocation = locationline /*&& itemGroup*/ ? returnAccounForLocation(locationline, prod_post_group) : null;
                    nlapiLogExecution("DEBUG", "acclocation <><>==", acclocation);

                    nlapiLogExecution("ERROR", "currentAcc===>", configData.custrecord_sdb_adj_acc_puente + ' -- <>' + currentAcc);

                    if (currentAcc == configData.custrecord_sdb_cuenta_costo) {

                        if (Number(standardLines.getLine(i).getCreditAmount()) > 0) {
                            nlapiLogExecution("DEBUG", "acclocation <>=<>", configData.custrecord_sdb_cuenta_costo);
                            ...
                            // if (entity && accType != 'AcctRec') newLine.setEntityId(parseInt(entity))
                            // if (standardLines.getLine(i).getSegmentValueId('cseg1')) newLine.setSegmentValueId('cseg1', standardLines.getLine(i).getSegmentValueId('cseg1'));
                            // if (isInterCompany != -1 && configData.custrecord_sdb_inv_cost_ic) newLine.setAccountId(parseInt(configData.custrecord_sdb_inv_cost_ic)); nlapiLogExecution("DEBUG", "configData.custrecord_sdb_inv_cost_i-1", configData.custrecord_sdb_inv_cost_ic);
                            // if (terceros != -1 && configData.custrecord_sdb_inv_cost_3ro) newLine.setAccountId(parseInt(configData.custrecord_sdb_inv_cost_3ro)); nlapiLogExecution("DEBUG", "configData.custrecord_sdb_inv_cost_3ro-1", configData.custrecord_sdb_inv_cost_3ro);
                        } else if (Number(standardLines.getLine(i).getDebitAmount()) > 0) {
                            nlapiLogExecution("DEBUG", "acclocation <>=<>2", configData.custrecord_sdb_cuenta_costo + ' -- ' + Number(standardLines.getLine(i).getDebitAmount()));
                            nlapiLogExecution("DEBUG", "currentAcc", currentAcc);
                            var newLine = customLines.addNewLine();
                            newLine.setCreditAmount(Number(standardLines.getLine(i).getDebitAmount()));
                            newLine.setAccountId(parseInt(currentAcc));
                            ...
                            if (entity && accType != 'AcctRec') newLine.setEntityId(parseInt(entity))
                            if (standardLines.getLine(i).getSegmentValueId('cseg1')) newLine.setSegmentValueId('cseg1', standardLines.getLine(i).getSegmentValueId('cseg1'));
                            //   if (isInterCompany != -1 && configData.custrecord_sdb_inv_cost_ic) newLine.setAccountId(parseInt(configData.custrecord_sdb_inv_cost_ic)); nlapiLogExecution("DEBUG", "configData.custrecord_sdb_inv_cost_i-2", configData.custrecord_sdb_inv_cost_ic);
                            if (terceros != -1 && configData.custrecord_sdb_inv_cost_3ro) newLine.setAccountId(parseInt(configData.custrecord_sdb_inv_cost_3ro)); nlapiLogExecution("ERROR", "configData.custrecord_sdb_inv_cost_3ro-2", configData.custrecord_sdb_inv_cost_3ro);
                        }
                    } else if (currentAcc == configData.custrecord_sdb_inv_item_acc /*|| articuloAcc.indexOf(currentAcc) != -1*/) {

                        if (Number(standardLines.getLine(i).getCreditAmount()) > 0) {
                            nlapiLogExecution("DEBUG", "getDebitAmount custrecord_sdb_inv_item_ac===", Number(standardLines.getLine(i).getCreditAmount()));
                            var newLine = customLines.addNewLine();
                            newLine.setDebitAmount(Number(standardLines.getLine(i).getCreditAmount()));
                            newLine.setAccountId(parseInt(currentAcc));
                            newLine.setLocationId(parseInt(currentLocation));
                            ...
                            if (entity && accType != 'AcctRec') newLine.setEntityId(parseInt(entity))
                            if (standardLines.getLine(i).getSegmentValueId('cseg1')) newLine.setSegmentValueId('cseg1', standardLines.getLine(i).getSegmentValueId('cseg1'));
                            if (isInterCompany != -1 && configData.custrecord_sdb_inv_art_intercom) newLine.setAccountId(parseInt(configData.custrecord_sdb_inv_art_intercom)); nlapiLogExecution("DEBUG", "configData.custrecord_sdb_inv_art_intercom-3", configData.custrecord_sdb_inv_art_intercom);
                            if (terceros != -1 && configData.custrecord_sdb_inv_art_3ro) newLine.setAccountId(parseInt(configData.custrecord_sdb_inv_art_3ro)); nlapiLogExecution("DEBUG", "configData.custrecord_sdb_inv_art_3ro-3", configData.custrecord_sdb_inv_art_3ro);
                        } else if (Number(standardLines.getLine(i).getDebitAmount()) > 0) {
                            nlapiLogExecution("DEBUG", "getDebitAmount custrecord_sdb_inv_item_acc===", Number(standardLines.getLine(i).getDebitAmount()));
                            var newLine = customLines.addNewLine();
                            newLine.setCreditAmount(Number(standardLines.getLine(i).getDebitAmount()));
                            ...
                            if (entity && accType != 'AcctRec') newLine.setEntityId(parseInt(entity))
                            if (standardLines.getLine(i).getSegmentValueId('cseg1')) newLine.setSegmentValueId('cseg1', standardLines.getLine(i).getSegmentValueId('cseg1'));
                            if (isInterCompany != -1 && configData.custrecord_sdb_inv_art_intercom) newLine.setAccountId(parseInt(configData.custrecord_sdb_inv_art_intercom)); nlapiLogExecution("DEBUG", "configData.custrecord_sdb_inv_art_intercom-4", configData.custrecord_sdb_inv_art_intercom);
                            if (terceros != -1 && configData.custrecord_sdb_inv_art_3ro) newLine.setAccountId(parseInt(configData.custrecord_sdb_inv_art_3ro)); nlapiLogExecution("DEBUG", "configData.custrecord_sdb_inv_art_3ro-4", configData.custrecord_sdb_inv_art_3ro);
                        } else if (Number(standardLines.getLine(i).getDebitAmount()) > 0) {
                        }
                        ...
                        if (Number(standardLines.getLine(i).getCreditAmount()) > 0) {
                            nlapiLogExecution("DEBUG", "getDeCreditAmount custrecord_sdb_inv_item_acc==", Number(standardLines.getLine(i).getCreditAmount()));
                            nlapiLogExecution("DEBUG", "acclocation==//++", acclocation);
                            ...
                            if (entity && accType != 'AcctRec') newLine.setEntityId(parseInt(entity))
                            if (standardLines.getLine(i).getSegmentValueId('cseg1')) newLine.setSegmentValueId('cseg1', standardLines.getLine(i).getSegmentValueId('cseg1'));
                            if (acclocation) newLine.setAccountId(parseInt(acclocation)); nlapiLogExecution("DEBUG", "acclocation-1", acclocation);
                        }

                    } else if (acclocation && currentAcc) {
                        ...
                        if (Number(standardLines.getLine(i).getCreditAmount()) > 0) {
                            ...
                            // if (isInterCompany != -1 && acclocation) newLine.setAccountId(parseInt(acclocation));
                            if (acclocation) newLine.setAccountId(parseInt(acclocation)); nlapiLogExecution("DEBUG", "acclocation2", acclocation);
                        } else if (Number(standardLines.getLine(i).getDebitAmount()) > 0) {
                            ...
                            if (standardLines.getLine(i).getSegmentValueId('cseg1')) newLine.setSegmentValueId('cseg1', standardLines.getLine(i).getSegmentValueId('cseg1'));
                            // if (isInterCompany != -1 && acclocation) newLine.setAccountId(parseInt(acclocation));
                            if (acclocation) newLine.setAccountId(parseInt(acclocation)); nlapiLogExecution("DEBUG", "acclocation3", acclocation);
                        }
                    }
                    // }
                }
                ...
                //     // nlapiLogExecution("DEBUG", "groupAccounts-IF", JSON.stringify(groupAccounts));
                //     // if (locationline) locationline = nlapiLookupField('location', locationline, 'name');
                //     var acclocation = returnAccounForLocation(locationline, prod_post_group)

                //     if (Number(standardLines.getLine(i).getCreditAmount()) > 0 && configData.custrecord_sdb_cuenta_costo == currentAcc) {//custrecord_sdb_inv_cost_3ro
                //         //     nlapiLogExecution("DEBUG", "getDebitAmount custrecord_sdb_inv_item_acc<->", Number(standardLines.getLine(i).getCreditAmount()));
                //         //     var newLine = customLines.addNewLine();
                //         //     newLine.setDebitAmount(Number(standardLines.getLine(i).getCreditAmount()));
                //         //     newLine.setAccountId(parseInt(currentAcc));
                //         //     newLine.setLocationId(parseInt(currentLocation));
                //         //     if (standardLines.getLine(i).getMemo()) newLine.setMemo('TEST 0')//newLine.setMemo(standardLines.getLine(i).getMemo())
                //         //     if (standardLines.getLine(i).getClassId()) newLine.setClassId(standardLines.getLine(i).getClassId());
                //         //     if (standardLines.getLine(i).getDepartmentId()) newLine.setDepartmentId(standardLines.getLine(i).getDepartmentId());
                //         //     if (entity && accType != 'AcctRec') newLine.setEntityId(parseInt(entity))
                //         //     if (standardLines.getLine(i).getSegmentValueId('cseg1')) newLine.setSegmentValueId('cseg1', standardLines.getLine(i).getSegmentValueId('cseg1'));
                ...
                //         //     newLine.setCreditAmount(amountValue);
                //         //     newLine.setLocationId(parseInt(currentLocation));
                //         //     if (standardLines.getLine(i).getMemo()) newLine.setMemo('TEST 1')//newLine.setMemo(standardLines.getLine(i).getMemo())
                //         //     if (standardLines.getLine(i).getClassId()) newLine.setClassId(standardLines.getLine(i).getClassId());
                //         //     if (standardLines.getLine(i).getDepartmentId()) newLine.setDepartmentId(standardLines.getLine(i).getDepartmentId());
                //         //     if (entity && accType != 'AcctRec') newLine.setEntityId(parseInt(entity))
                //         //     if (standardLines.getLine(i).getSegmentValueId('cseg1')) newLine.setSegmentValueId('cseg1', standardLines.getLine(i).getSegmentValueId('cseg1'));
                ...
                //         // newLine.setDebitAmount(amountValue);
                //         // newLine.setLocationId(parseInt(currentLocation));
                //         // if (standardLines.getLine(i).getMemo()) newLine.setMemo('TEST 2')//newLine.setMemo(standardLines.getLine(i).getMemo())
                //         // if (standardLines.getLine(i).getClassId()) newLine.setClassId(standardLines.getLine(i).getClassId());
                //         // if (standardLines.getLine(i).getDepartmentId()) newLine.setDepartmentId(standardLines.getLine(i).getDepartmentId());
                //         // if (entity && accType != 'AcctRec') newLine.setEntityId(parseInt(entity))
                //         // if (standardLines.getLine(i).getSegmentValueId('cseg1')) newLine.setSegmentValueId('cseg1', standardLines.getLine(i).getSegmentValueId('cseg1'));
                ...
                //         if (Number(standardLines.getLine(i).getCreditAmount()) > 0) {
                //             nlapiLogExecution("DEBUG", "getDebitAmount custrecord_sdb_inv_item_acc <><>//", Number(standardLines.getLine(i).getCreditAmount()));
                //             nlapiLogExecution("DEBUG", "acclocation <><>1", acclocation);
                //             var newLine = customLines.addNewLine();
                //             newLine.setDebitAmount(Number(standardLines.getLine(i).getCreditAmount()));
                //             newLine.setAccountId(parseInt(currentAcc));
                //             newLine.setLocationId(parseInt(currentLocation));
                ...
                //         } else if (Number(standardLines.getLine(i).getDebitAmount()) > 0) {
                //             nlapiLogExecution("DEBUG", "acclocation <><>2", acclocation);
                //             nlapiLogExecution("DEBUG", "getDebitAmount custrecord_sdb_inv_item_acc <><>||", Number(standardLines.getLine(i).getDebitAmount()));
                //             var newLine = customLines.addNewLine();
                //             newLine.setCreditAmount(Number(standardLines.getLine(i).getDebitAmount()));
                //             newLine.setAccountId(parseInt(currentAcc));
    ...
    //SECTOR DE FUNCIONES AUXILIARES
    ...
    function returnAccounForLocation(locationline, posgroup) {
        var account;
        try {
            var filter = [
                ["custrecord_sdb_location", "anyof", locationline],
                "AND",
                ["custrecord_sdb_inv_posting_group", "anyof", posgroup]
            ]
            if (!posgroup) {
                filter = [
                    ["custrecord_sdb_location", "anyof", locationline]
            ...
            var result = nlapiSearchRecord("customrecord_sdb_inventory_account", null,
                filter,
                [
                    new nlobjSearchColumn("scriptid").setSort(false),
                    new nlobjSearchColumn("custrecord_sdb_location"),
                    new nlobjSearchColumn("custrecord_sdb_accounts")
                ]
            );

            if (result) account = result[0].getValue('custrecord_sdb_accounts')

        } catch (e) {
            nlapiLogExecution("ERROR", "returnAccounForLocation", e);
        }
    ...
    function setCustGroupAccount(postGroup, nsAcc) {

        try {
            var results = nlapiSearchRecord("customrecord_sdb_cust_posting_group_acc", null,
                [
                    ["custrecord_sdb_cust_post_group_df_acc", "anyof", nsAcc],
                    "AND",
                    ["custrecord_sdb_cust_posting_group", "anyof", postGroup]
                ],
                [
                    new nlobjSearchColumn("custrecord_sdb_cust_post_group_acc_set")
                ]
            );


            if (results) return results[0].getValue('custrecord_sdb_cust_post_group_acc_set');


        } catch (e) {
            nlapiLogExecution("ERROR", "setCustGroupAccount", e);
        }
        return false
    }

    function setGeneralGroupAccount(postGroup, nsAcc, gen_prod_pg) {

        try {
            nlapiLogExecution("audit", "gen_prod_pg", JSON.stringify(gen_prod_pg));
            var filter = [];
            if (gen_prod_pg) {
                filter = [
                    ["custrecord_sdb_deafult_acc_ns", "anyof", nsAcc],
                    "AND",
                    ["custrecord_sdb_posting_group", "anyof", postGroup],
                    "AND",
                    ["custrecord_sdb_gen_prod_posting_group", "anyof", gen_prod_pg]
                ]
            } else {
                filter = [
                    ["custrecord_sdb_deafult_acc_ns", "anyof", nsAcc],
            ...
            var results = nlapiSearchRecord("customrecord_sdb_gen_prod_posting_group", null,
                filter,
                [
                    new nlobjSearchColumn("custrecord_sdb_sales_account"),
                    new nlobjSearchColumn("custrecord_sdb_sales_disc_account"),
                    new nlobjSearchColumn("custrecord_sdb_acc_with_installation"),
                    new nlobjSearchColumn("custrecord_sdb_acc_with_installation"),
                    new nlobjSearchColumn("custrecord_sdb_is_warranty"),
                    new nlobjSearchColumn("custrecord_sdb_gen_discount_acc")
                ]
            );

            if (results) {
                return {
                    salesAcc: results[0].getValue('custrecord_sdb_sales_account'),
                    discAcc: results[0].getValue('custrecord_sdb_sales_disc_account'),
                    accWithInstall: results[0].getValue('custrecord_sdb_acc_with_installation'),
                    isWarranty: results[0].getValue('custrecord_sdb_is_warranty'),
                    discountAcc: results[0].getValue('custrecord_sdb_gen_discount_acc')
                }
            }
    ...
    function getAccCost(postGroup, nsAcc) {
        try {
            var results = nlapiSearchRecord("customrecord_sdb_cost_account_mapper", null,
                [
                    ["custrecord_sdb_cost_account_df_ns", "anyof", nsAcc],
                    "AND",
                    ["custrecord_sdb_map_cost_post_group", "anyof", postGroup]
                ],
                [
                    new nlobjSearchColumn("scriptid").setSort(false),
                    new nlobjSearchColumn("custrecord_sdb_map_cost_post_group"),
                    new nlobjSearchColumn("custrecord_sdb_cost_account_df_ns"),
                    new nlobjSearchColumn("custrecord_sdb_cost_account_map")
                ]
            );
            if (results) return results[0].getValue('custrecord_sdb_cost_account_map');
        } catch (e) {
            nlapiLogExecution("ERROR", "getAccCost", e);
        }
        return false
    }

    function getChargeAcc(group, current) {
        try {
            var results = nlapiSearchRecord("customrecord_sdb_charge_account_mapper", null,
                [
                    ["custrecord_sdb_charge_post_group", "anyof", "1", group],
                    "AND",
                    ["custrecord_sdb_charge_account_mapper", "anyof", current]
                ],
                [
                    new nlobjSearchColumn("scriptid").setSort(false),
                    new nlobjSearchColumn("custrecord_sdb_charge_account_mapper"),
                    new nlobjSearchColumn("custrecord_sdb_charge_acc_to_set"),
                    new nlobjSearchColumn("custrecord_sdb_charge_post_group")
                ]
            );
            if (results) return results[0].getValue('custrecord_sdb_charge_acc_to_set');

        } catch (e) {
            nlapiLogExecution("ERROR", "getChargeAcc", e);
        }
        return false
    }

    function getItemPostGroup(item) {
        try {

            var itemData = nlapiLookupField('item', item, ['custitem_sdb_gen_prod_post_group', 'recordType']);
            return itemData

        } catch (e) {
            nlapiLogExecution("ERROR", "getItemPostGroup", e);
        }
        ...
        try {
            ...
            return (results || []).map(function (line) {
                return {
                    ...
                    item: line.getValue('internalid', 'item'),
                    ...
                    cusScript: line.getValue('customscript')
                };
            });
        } catch (e) {
    ...
    function getReport(createdfrom, item) {

        try {

            var filters = [
                ["custrecord_sdb_sales_order_rec.custrecord_sdb_item_rec", "anyof", item],
                "AND",
                ["custrecord_sdb_sales_order_rec.custrecord_sdb_report_number", "isnotempty", ""],
                "AND",
                ["internalid", "anyof", createdfrom],
                "AND",
                ["custrecord_sdb_sales_order_rec.custrecord_sdb_installed_rec", "is", "T"]
            ]

            var results = nlapiSearchRecord('transaction', null, filters,
                [

                    new nlobjSearchColumn("custrecord_sdb_instaltion_date_rec", "CUSTRECORD_SDB_SALES_ORDER_REC", null),
                    new nlobjSearchColumn("custrecord_sdb_item_rec", "CUSTRECORD_SDB_SALES_ORDER_REC", null),
                    new nlobjSearchColumn("custrecord_sdb_report_number", "CUSTRECORD_SDB_SALES_ORDER_REC", null),
            ...
            return (results || []).map(function (line) {
                return {
                    dateInstalled: line.getValue("custrecord_sdb_instaltion_date_rec", "CUSTRECORD_SDB_SALES_ORDER_REC"),
                    item: line.getValue("custrecord_sdb_item_rec", "CUSTRECORD_SDB_SALES_ORDER_REC"),
                    report: line.getValue("custrecord_sdb_report_number", "CUSTRECORD_SDB_SALES_ORDER_REC")
                };
            });
        } catch (e) {
            nlapiLogExecution("ERROR", "getReport", e);
        }
    }
}
